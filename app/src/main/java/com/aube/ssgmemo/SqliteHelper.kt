...
class SqliteHelper(context: Context, name: String, version: Int) :
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        db?.execSQL(sql)
        val sql1 =
            "create table memo (idx integer primary key, title text, content text, contentString text, contentAttribute text, datetime integer, ctgr integer, priority integer, status integer, FOREIGN KEY (ctgr) references ctgr(idx))"
        db?.execSQL(sql1)
    }
    ...
    fun insertMemo(memo: Memo) {
        val wd = writableDatabase
        val sql =
            "insert into memo (title, content, contentString, contentAttribute, datetime, ctgr, priority, status) values " +
                    ...
                    }', '${memo.contentAttribute}', ${memo.datetime}, ${memo.ctgr}, ${memo.priority}, ${memo.status})"
        wd.execSQL(sql)
        wd.close()
    }
    ...
    fun movePriority(itemList_from: Memo, itemList_to: Memo) {
        ...
        val sql =
            "update memo set priority = '" + itemList_from.priority + "' where idx = '" + itemList_from.idx + "'"
        val sql1 =
            "update memo set priority = '" + itemList_to.priority + "' where idx = '" + itemList_to.idx + "'"
    ...
    fun sortPriority(cidx: Int?) {
        ...
        while (rs.moveToNext()) {
            val sql2 =
                "update memo set priority = '$priority', ctgr = $cidx where idx = '" + rs.getLong(
                    rs.getColumnIndex("idx")
                ) + "'"
            wd.execSQL(sql2)
            priority++
        }
    ...
    fun selectMemoList(ctgr: String): MutableList<Memo> {
        ...
        val list = mutableListOf<Memo>()
        val sql =
            "select * from memo where ctgr = '$ctgr' and status = 1 order by priority desc"
        val rd = readableDatabase
        val rs = rd.rawQuery(sql, null)
    ...
    fun selectSearchList(keyword: String, where: String, orderby: String): MutableList<Memo> {
        ...
        when (where) {
            ...
            "제목+내용" -> {
                condition1 =
                    "where (title like '%$keyword%' or contentString like '%$keyword%') and status = 1"
            }
        }
    ...
    fun selectCompleteList(keyword: String, where: String, orderby: String): MutableList<Any> {
        ...
        when (where) {
            ...
            "제목+내용" -> {
                condition1 =
                    "where (title like '%$keyword%' or contentString like '%$keyword%') and status = 2"
            }
        }
        ...
        for (i in list) {
            ...
            val date2 = if (num + 1 != list.size) {
                dateFormat.format(Date(list.get(num + 1).datetime))
            } else {
                "0"
            }
    ...
    fun deleteMemo(memo: Memo) { // 휴지통에서 메모 삭제
        ...
        val wd1 = writableDatabase
        val sql1 =
            "update memo set priority = priority - 1 where ctgr = '" + memo.ctgr + "' and priority>'" + memo.priority + "'"
        wd1.execSQL(sql1)
        wd1.close()
    ...
    fun deleteMemoCtgr(memo: Memo) { // ?????????????????????
        val priority = checkTopPriority(0) + 1
        val wd = writableDatabase
        val sql1 =
            "update memo set priority = priority - 1 where ctgr = '" + memo.ctgr + "' and priority >'" + memo.priority + "'"
        val sql2 =
            "update memo set priority = '$priority', ctgr = 0 where idx = '${memo.idx}'"
        wd.execSQL(sql1)
        wd.execSQL(sql2)
    ...
    fun deleteCtgr(ctgrIdx: String) {
        ...
        for (memo in memoList) {
            var sql3 =
                "update memo set priority = '" + ++unclassifiedTop + "' where idx = '" + memo.idx + " '"
            wd.execSQL(sql3)
        }
        wd.close()
    }
    ...
    fun checkTopPriority(ctgr: Int): Int {
        ...
        val sql =
            "select priority from memo where ctgr = '$ctgr' order by priority desc limit 1"
        val rd = readableDatabase
        val rs = rd.rawQuery(sql, null)
        var result = 0
    ...
    fun checkMemoListSize(ctgrIdx: Int?): Int {
        // 카테고리에 몇 개의 메모가 있는지
